...
export const logo = {
  ...
  "rules": [
    "The circle stroke is exactly half the triangle and crossbar weight — 15 against 30.",
    "The triangle's base is exactly the circle diameter, 485, and its corners land on the circle's left and right extremes (x 91 and 576). The base is the diameter laid flat. This is a relationship the previous revision did not have and is the clearest thing to check a redraw against.",
    "All three triangle points sit outside the circle. The apex clears the top of the arc by 37.5, and each base corner breaks through the lower arc by 44.5 — at y 450 the circle is only 396 wide against the triangle's 485.",
    "The crossbar runs the full width and overhangs the circle by 0.38r each side, starting exactly at the circle's left extreme. Never clip the bar to the circle — the overhang is the mark's distinguishing move.",
    "The crossbar is centred on the circle's horizontal centreline: it spans y 295–325, midpoint 310, which is cy exactly.",
    "Aspect is 1.19:1, not square. Set height and let width follow. Anything carrying 1.17:1, 1.24:1 or 1.28:1 is stale — 1.24:1 was this mark's own previous revision.",
  ...
  "wordmark": {
    "_note": "Typesetting for the two files that carry live text. The standalone wordmark's fontSize and canvas are declared — it stands alone, with no mark to be proportional to. The lockup's fontSize is DERIVED from lockup.widthOfDiameter and the metrics below, so it holds its proportion when the mark is redrawn. letterSpacing is derived from scale.tracking.wordmark, which is why the two files can no longer disagree the way they once did.",
    "text": "ARCHETYPE",
    "fontSize": 58,
    "fontWeight": 700,
    "canvas": {
    ...
    "metrics": {
      "_note": "MEASURED, not estimated — and measured from a real render, which is the only method that proved trustworthy. Canvas measureText() disagreed with what the browser actually drew, so these come from rasterising the lockup at 3x with the Typekit faces loaded and reading the ink out of the pixels. Face-dependent: re-measure if the display family changes. The figures they replaced were both wrong — the lockup's hand-set fontSize of 58 rendered the wordmark at 0.728 of the circle diameter while tokens.json claimed 0.76.",
      "advanceRatio": 6.148,
      "advanceRatioNote": "Width of the string ARCHETYPE per 1px of font-size, at 0.02em tracking. Excludes the trailing tracking gap CSS adds after the last glyph and SVG does not render.",
      "inkAscentRatio": 0.734,
      "inkAscentNote": "Ink top above the baseline, per 1px of font-size. Larger than the face's cap height because the round C overshoots, and it is the ink the eye reads, so it is what places the lockup baseline.",
      "inkDescentRatio": 0.011,
      "inkDescentNote": "All-caps has no descenders; this is the round letters dipping below the baseline."
    }
  },
  "lockup": {
    "_note": "The stacked lockup: mark above, wordmark beneath. Both fractions are of the circle diameter, so the lockup rescales with the mark rather than with a fixed pixel gap. widthOfDiameter now actually drives the wordmark's font size — it was previously a claim nothing enforced.",
    "widthOfDiameter": 0.76,
    "gapOfDiameter": 0.08,
    "bottomPad": 8,
    "bottomPadNote": "Breathing room below the wordmark's ink, in the mark's own coordinate space."
  }
};
